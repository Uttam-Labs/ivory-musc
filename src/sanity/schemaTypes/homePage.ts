  ...
  fields: [
    ...
    defineField({ name: "sections", title: "Page sections", type: "array", of: [
      defineArrayMember({ name: "hero", title: "Hero banner", type: "object", fields: [
        ...
        defineField({ name: "videoMp4", title: "Hero video (MP4)", description: "Optional. Leave empty to keep the current optimised live video.", type: "file", options: { accept: "video/mp4" } }),
        defineField({ name: "videoWebm", title: "Hero video (WebM)", description: "Optional WebM version for faster loading. Leave empty to keep the current live file.", type: "file", options: { accept: "video/webm" } }),
        defineField({ name: "videoPoster", title: "Hero video poster", description: "Optional image displayed while the video loads.", type: "image", options: { hotspot: true }, fields: [defineField({ name: "alt", type: "string" })] }),
        imageField,
        ...linkFields,
      ...
      defineArrayMember({ name: "featureGuide", title: "Feature guide", type: "object", fields: [defineField({ name: "heading", type: "string" }), defineField({ name: "body", type: "text" }), ...linkFields, defineField({ name: "features", type: "array", of: [defineArrayMember({ type: "object", fields: [defineField({ name: "title", type: "string" }), defineField({ name: "icon", type: "image" }), defineField({ name: "showOnWebsite", title: "Show on website", description: "Turn this off to hide this symbol and its label from the website without deleting it. Turn it back on and publish when the symbol can be displayed again.", type: "boolean", initialValue: true })] })] })] }),
      defineArrayMember({ name: "newsletter", title: "Newsletter", type: "object", fields: [
        defineField({ name: "heading", type: "string" }),
        defineField({ name: "body", type: "text" }),
        ...
        defineField({ name: "submittingLabel", title: "Submitting text", type: "string", initialValue: "Subscribing…" }),
