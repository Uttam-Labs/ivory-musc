...
function key() {
  if (!env.CUSTOMER_ACCOUNT_SESSION_SECRET) throw new Error("Customer account session secret is not configured");
  return createHash("sha256").update(env.CUSTOMER_ACCOUNT_SESSION_SECRET).digest();
}
...
function decryptSession(value: string): CustomerSession | null {
  try {
    ...
    const session = JSON.parse(Buffer.concat([decipher.update(encrypted), decipher.final()]).toString("utf8")) as CustomerSession;
