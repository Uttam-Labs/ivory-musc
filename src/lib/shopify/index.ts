...
export async function getProducts(first = 12, query?: string) {
  const [data, hiddenHandles] = await Promise.all([
    shopifyFetch<{ products: { nodes: Product[] } }>({ query: PRODUCTS_QUERY, variables: { first, query }, tags: ["shopify", "products"] }),
    getHiddenProductHandles(),
  ]);
  return data.products.nodes.filter((product) => !isProductHidden(product, hiddenHandles));
}
...
export async function getProductRecommendations(productId: string) {
  const [data, hiddenHandles] = await Promise.all([
    shopifyFetch<{ productRecommendations: Product[] }>({
  ...
  return data.productRecommendations.filter((product) => !isProductHidden(product, hiddenHandles));
}
...
export async function getCollection(handle: string, first = 24) {
  const [data, hiddenHandles] = await Promise.all([
    shopifyFetch<{ collection: (Collection & { products: { nodes: Product[] } }) | null }>({ query: COLLECTION_QUERY, variables: { handle, first }, tags: ["shopify", `collection:${handle}`] }),
    getHiddenProductHandles(),
  ]);
  ...
  return {
    ...
    products: {
      ...
      nodes: data.collection.products.nodes.filter((product) => !isProductHidden(product, hiddenHandles)),
    },
  };
}
