...
export default async function SilkGuidePage(){
  ...
  return <main className={`${styles.page} silk-guide-page`}>
    ...
    {visible.cta!==false&&hasCta&&<section className={styles.cta}><span>{page.cta?.eyebrow}</span><h2>{page.cta?.heading}</h2><Paragraphs text={page.cta?.body}/>{page.cta?.buttonLabel&&page.cta?.buttonHref&&<Link href={page.cta.buttonHref}>{page.cta.buttonLabel}</Link>}</section>}
  </main>
}
