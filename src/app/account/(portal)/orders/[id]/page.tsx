...
export default async function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <header className={`${styles.header} ${styles.orderDetailHeader}`}>
      ...
      <div className={styles.orderDetailActions}>{order.canceledAt && <span className={`${styles.orderBadge} ${styles.orderBadge_danger}`}>{c.cancelledBadgeLabel}</span>}<span className={`${styles.orderBadge} ${styles[`orderBadge_${financialTone(order.financialStatus)}`]}`}>{readable(order.financialStatus)}</span>{!order.canceledAt && <span className={`${styles.orderBadge} ${styles[`orderBadge_${fulfillmentTone(order.fulfillmentStatus, false)}`]}`}>{readable(order.fulfillmentStatus)}</span>}</div>
    </header>
      ...
      <aside className={styles.orderDetailSidebar}>
        <section className={`${styles.orderInfoCard} ${styles.orderActionsCard}`}><div className={styles.orderSectionTitle}><CreditCard size={22} /><h2>{c.paymentDetailsHeading}</h2></div>
          <dl className={styles.paymentSummary}>
            <div><dt>{c.paymentMethodLabel}</dt><dd>{paymentSummary?.method || paymentSummary?.gateway || c.paymentUnavailableText}</dd></div>
            {paymentSummary?.maskedNumber && <div><dt>{c.paymentCardLabel}</dt><dd>{paymentSummary.maskedNumber}</dd></div>}
            {paymentSummary?.gateway && paymentSummary.gateway !== paymentSummary.method && <div><dt>{c.paymentProviderLabel}</dt><dd>{paymentSummary.gateway}</dd></div>}
            <div><dt>{c.paymentStatusLabel}</dt><dd>{readable(order.financialStatus)}</dd></div>
            <div><dt>{c.paymentAmountLabel}</dt><dd>{formatMoney(paymentSummary?.amount || order.totalPrice)}</dd></div>
          </dl>
        ...
        <section className={styles.orderInfoCard}><div className={styles.orderSectionTitle}><Truck size={22} /><h2>{c.trackingHeading}</h2></div>{tracking.length ? tracking.map((item, index) => <div className={styles.trackingItem} key={`${item.number}-${index}`}><span className={styles.trackingCarrier}>{item.company || c.carrierLabel}</span>{item.number && <strong>{item.number}</strong>}{item.url && <a href={item.url} target="_blank" rel="noopener noreferrer">{c.trackPackageLabel}<ExternalLink size={14} /></a>}</div>) : <p className={styles.muted}>{c.trackingPendingText}</p>}</section>
